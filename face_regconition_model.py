...
class IResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.drop_lastfc:
            x = self.dropout(
                x)  # if you train with the metric loss(e.g. sn-pair, n-pair) on MS1M-R or MS1M-R+T4, don't use dropout
...
def iresnet(num_layers, drop_lastfc=True):
    if num_layers == 34:
...
if __name__ == '__main__':
    model = iresnet(100)
    model.to('cuda')
    model.load_state_dict(torch.load('checkpoint/resnet100.pth'))
    model.eval()